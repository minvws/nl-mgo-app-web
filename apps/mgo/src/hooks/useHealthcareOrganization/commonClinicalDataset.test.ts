...
test('returns the common clinical dataset client when there is a resource endpoint available', () => {
    ...
    const commonClinicalDataset = getCommonClinicalDataset(organization);
...
test('returns NULL when there is NO organization', () => {
    const commonClinicalDataset = getCommonClinicalDataset(undefined);
