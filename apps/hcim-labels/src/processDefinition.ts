import { type StructureDefinition } from '@minvws/mgo-fhir';
 ...
 */
function snakeCase(value: string | undefined): string {
    if (!value) return '';
    return value.replace(/[^.]+/g, (match) => lodashSnakeCase(match));
}
