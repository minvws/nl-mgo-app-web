...
A schema that describes a set of ui elements containing health care data. This is used to ensure all client applications show the same information. It also prevents the need for multiple implementations for parsing FHIR data per platform (`web`/`ios`/`android`). See the `fhir-data` package for more information
