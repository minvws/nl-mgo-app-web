...
const mockIntl = {
    locale: Locale.NL_NL,
    formatMessage: vi.fn((value: MessageDescriptor, values: unknown) =>
        values ? `intl(${value.id}, ${JSON.stringify(values)})` : `intl(${value.id})`
    ),
...
export const uiHelperContext = createMockDataFactory<UiHelperContext>(() => {
    return {
        ...
        formatMessage: (id: MessagesIds, _values: unknown) =>
            mockIntl.formatMessage({ id }, _values),
