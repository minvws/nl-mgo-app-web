...
test('format dateTime returns undefined for nullish', () => {
    ...
    const result = date(faker.ui.context())(value);
    expect(result).toBeUndefined();
});
test('format dateTime returns empty string for empty strings', () => {
    const result = date(faker.ui.context())('' as DateTimeString);
    expect(result).toBe('');
});
...
test.each<[string, string]>([
    ...
    const result = date(faker.ui.context())(dateTimeString as DateTimeString);
