...
test('returns the expected output 01', () => {
    const output = zibProcedure.parse(input01 as Procedure, faker.custom.i18nContext());
    expectJson(output).toMatchFileSnapshot('./fixtures/01/mgo-resource.snap.json');
});

test('returns the expected output 02', () => {
    const output = zibProcedure.parse(input02 as Procedure, faker.custom.i18nContext());
